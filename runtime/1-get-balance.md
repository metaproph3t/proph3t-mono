*In this series, we're going to dissect the Solana runtime's internals. By the end, you should have a decent grasp of how Solana processes transactions and responds to queries. Each post will take about 10 minutes to read. The posts are targeted towards a technical audience.*
...
The lamport balance of the account is contained in the `value` field. Observe that the response also contains a few pieces of ancillary data, such as `apiVersion` and `slot`.
...
Once your request makes its away across the internet, the validator passes it to `JsonRpcRequestProcessor`. Since you specified `getBalance` in the `method` field, it passes the request to `JsonRpcRequestProcessor`'s `get_balance` function, depicted below.
...
Suppose that we are building a simple blockchain that keeps track of native token (SOL, ETH, BTC, etc.) balances. Below is an example of a sequence of blocks that could occur on our blockchain.

![Two blocks after genesis](media/simple-blockchain-1.png)

Here, the ledger would look like:
```
[
...
Assumedly, Alice had some starting balance that allowed her to fund her transfer to Bob; let us say that she started with 100. After the second block has been processed, the state should look like the following:
...
At this point, the blockchain has *two potential states*. It needs to either store or be able to construct both, because the blockchain needs this state to validate future transactions. Here, this is making sure that someone doesn't spend more than their balance.

Bringing it back to Solana, a `Bank` is just a state at a given block. Remember that `RpcContextConfiguration` object that gets passed into `JsonRpcRequestProcessor`? This specifies which bank to process the request against. Since we didn't specify any in our API request, it used the default, which is the latest bank that has been approved by 2/3rds of the active stake.

Now let us look at the code, in two stages. Here is the `Bank`'s `get_balance` function that `JsonRpcRequestProcessor` called:
...
Now we can shift our attention to the Bank's `get_account` function:
...
The last step along our journey is the actual state database, which is encapsulated in the `Accounts` and `AccountsDb`. These objects are too low-level for a 10-minute blog post, so we will eschew the code in favor of a conceptual understanding of what these components do. 
...
Then, when a bank becomes finalized by 2/3rds of the state voting for it, account updates for that bank and any of the bank's ancestors get flushed from memory to disk.

For example, let us say that 2/3rds of validators vote for block three, which finalizes it and means that block two can never be added to the chain.

![Fork with three gets rooted, fork with two gets pruned](media/rooted-pruned-solana-forks.excalidraw.png)

At this point, account updates from block three and its ancestors (in this case, just block one) get flushed to disk and account updates from blocks earlier than block three but aren't an ancestor of block three (in this case, just block two), get pruned.
